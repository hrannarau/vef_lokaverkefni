...
def checkout():
    ...
    connect = pymysql.connect(user="1612002390", passwd="mypassword", host="tsuts.tskoli.is", port=3306,
                              database="1612002390_veflokaverkefni")
    upphaed = 0
    with connect.cursor() as cursor:
        sql = "select price from bilar where id = %s"
        for i in shoppingCart:
            if cursor.execute(sql,(i)):
                utkoma = cursor.execute(sql, (i))
                a = utkoma[0]
                return str(a)
